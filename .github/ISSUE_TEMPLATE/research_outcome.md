---
name: Research Outcome
about: Define a research question with evidence requirements
title: "[RESEARCH] "
labels: ''
assignees: ''
---

## Phase

Phase A — Evidence Lock

## Owner Agent

- [ ] agent:orchestrator
- [ ] agent:regulatory
- [ ] agent:cv
- [ ] agent:security

## Research Question

<!-- The specific question this research outcome answers -->

## Scope and Boundaries

<!-- What is in scope and out of scope for this research -->

### In Scope

-

### Out of Scope

-

## Acceptance Criteria

<!-- What constitutes a complete answer? -->
- [ ] Criterion 1
- [ ] Criterion 2

## Required Citations

<!-- Minimum citation requirements -->
- [ ] Minimum 3 primary sources per major claim
- [ ] Sources dated within 24 months
- [ ] Government/regulatory sources for compliance claims
- [ ] Peer-reviewed publications for accuracy claims

## Evidence Bundle Requirements

- [ ] Written summary with structured findings
- [ ] Source citation table (URL, date, jurisdiction, verification)
- [ ] Confidence assessment (high/medium/low)
- [ ] Identified gaps or unknowns

## Dependencies

- Depends on #

## Risk Classification

- [ ] risk:r1
- [ ] risk:r2
- [ ] risk:r3
- [ ] risk:r4

## Region Tags

- [ ] region:us
- [ ] region:eu
- [ ] region:uk
