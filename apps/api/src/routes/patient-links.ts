...
patientLinksRouter.get('/:token', async (req, res, next) => {
  try {
    ...
    if (new Date() > new Date(link.expiresAt)) { res.status(410).json({ error: 'This link has expired' }); return; }
...
patientLinksRouter.post('/:token/measurement', validateBody(measurementSchema), async (req, res, next) => {
  try {
    const token = req.params.token;
    if (!token || typeof token !== 'string') {
      res.status(400).json({ error: 'Invalid token parameter' });
    ...
    const link = await patientLinks.getByToken(token);
    if (!link) { res.status(404).json({ error: 'Link not found' }); return; }
    if (link.usedAt) { res.status(409).json({ error: 'Already used' }); return; }
    if (new Date() > new Date(link.expiresAt)) { res.status(410).json({ error: 'Expired' }); return; }
