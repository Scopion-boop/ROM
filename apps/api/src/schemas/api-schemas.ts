...
export const updateNoteStatusSchema = z.object({
  status: z.enum(['draft', 'reviewed', 'finalized']).describe('Note status for clinical workflow'),
});
...
export const registerSchema = z
  .object({
    ...
    role: z
      .enum(['clinician', 'admin', 'viewer', 'clinic_admin', 'reviewer', 'support_readonly'])
      .default('clinician')
      .describe('User role'),
  })
  .refine((data) => data.organizationId || data.clinicName || data.inviteToken, {
