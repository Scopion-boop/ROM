...
export function PhoneCameraLink({
  ...
  return (
    <div className={`flex flex-col items-center gap-4 rounded-xl bg-gray-800 p-6 ${className}`}>
      <h3 className="text-sm font-semibold text-gray-300">Pair Phone Camera (Optional)</h3>
      ...
      <div className="flex h-48 w-48 items-center justify-center rounded-lg bg-white p-3">
        {phoneUrl ? (
          <QRCodeSVG
            ...
            level="M"
            role="img"
            aria-label="QR code to pair phone camera"
          />
        ) : (
      ...
      <p className="text-xs text-gray-500 text-center max-w-[220px]">
        Scan the QR code with your phone to use its camera as a secondary view. The phone camera
        provides a different angle for more accurate 3D measurements.
      </p>
    </div>
  );
}
