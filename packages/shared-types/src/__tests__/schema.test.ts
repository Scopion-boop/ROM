...
describe('SessionSchema', () => {
    it('accepts a valid session', () => {
        const valid = {
            id: UUID1, organizationId: UUID2, clinicianId: UUID3,
            status: 'created', joints: ['shoulder'],
    ...
    it('rejects invalid session status', () => {
        const invalid = {
            id: UUID1, organizationId: UUID2, clinicianId: UUID3,
            status: 'nonexistent_status', joints: ['knee'],
            createdAt: TS, updatedAt: TS,
        };
        expect(() => SessionSchema.parse(invalid)).toThrow();
    });
    ...
    it('accepts multiple valid joint types', () => {
        const valid = {
            id: UUID1, organizationId: UUID2, clinicianId: UUID3,
            status: 'created', joints: ['shoulder', 'knee', 'cervical_spine'],
            createdAt: TS, updatedAt: TS,
        };
        expect(SessionSchema.parse(valid)).toBeDefined();
    });
    ...
    it('rejects invalid joint type in array', () => {
        const invalid = {
            id: UUID1, organizationId: UUID2, clinicianId: UUID3,
            status: 'created', joints: ['shoulder', 'finger'],
            createdAt: TS, updatedAt: TS,
        };
        expect(() => SessionSchema.parse(invalid)).toThrow();
    });
...
describe('CreateSessionSchema', () => {
    it('accepts valid create session input', () => {
        const valid = {
            organizationId: UUID2, clinicianId: UUID3,
...
describe('MeasurementSchema', () => {
    const baseMeasurement = {
        id: UUID1, sessionId: UUID2,
        joint: 'knee', movement: 'flexion', side: 'right',
        romDegrees: 135.5, confidenceScore: 0.92,
        qualityFlags: [], algorithmVersion: 'v1.0',
        captureDurationMs: 3200, createdAt: TS,
    };
    ...
    it('accepts midline side for spine', () => {
        const spine = {
            ...baseMeasurement,
            joint: 'lumbar_spine', movement: 'flexion', side: 'midline',
        };
    ...
    it('accepts optional normalRomDegrees and percentOfNormal', () => {
        const withNorm = {
            ...baseMeasurement,
            normalRomDegrees: 140, percentOfNormal: 97,
        };
...
describe('NoteBlockSchema', () => {
    it('accepts a valid note block', () => {
        const valid = {
            joint: 'shoulder', movement: 'flexion', side: 'left',
            romDegrees: 160, confidenceScore: 0.95,
        };
    ...
    it('accepts optional normative fields', () => {
        const valid = {
            joint: 'knee', movement: 'flexion', side: 'right',
            romDegrees: 130, confidenceScore: 0.9,
            normalRomDegrees: 140, percentOfNormal: 93,
        };
    ...
    it('accepts midline for spine blocks', () => {
        const valid = {
            joint: 'cervical_spine', movement: 'flexion', side: 'midline',
            romDegrees: 45, confidenceScore: 0.88,
        };
        expect(NoteBlockSchema.parse(valid)).toBeDefined();
    });
});
...
describe('NoteSchema', () => {
    const baseNote = {
        id: UUID1, sessionId: UUID2, clinicianId: UUID3,
        status: 'draft',
        blocks: [
...
describe('AuditEventSchema', () => {
    it('accepts a valid audit event', () => {
        const valid = {
            id: UUID1, action: 'session.created', actorId: UUID3,
            organizationId: UUID2, resourceType: 'session',
            resourceId: UUID1, timestamp: TS,
        };
...
describe('UserSchema', () => {
    it('accepts a valid user', () => {
        const valid = {
            id: UUID3, organizationId: UUID2,
            email: 'clinician@clinic.example',
            displayName: 'Dr. Smith', role: 'clinician',
